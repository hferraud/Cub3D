...
static void	draw_health_bar_pixel(t_cub *cub, t_vector offset, t_vector screen)
{
	...
	if (!is_in_border_radius(offset, BORDER_RADIUS - BORDER_SIZE))
		color = pixel_transparency(cub, screen, HB_BORDER_COLOR);
	else
	{
		...
		if ((offset.x < BORDER_SIZE
				|| offset.x >= HEALTH_BAR_WIDTH - BORDER_SIZE)
			|| (offset.y < BORDER_SIZE
				|| offset.y >= HEALTH_BAR_HEIGHT - BORDER_SIZE))
			color = pixel_transparency(cub, screen, HB_BORDER_COLOR);
		else if (x_ratio < life_ratio)
			color = HB_HEALTH_COLOR;
		else
			color = pixel_transparency(cub, screen, HB_EMPTY_COLOR);
	}
