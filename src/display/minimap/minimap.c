...
void	draw_minimap(t_cub *cub)
{
	...
	square = set_square(set_point((int) cub->map->spawn.x * wall_width,
				(int) cub->map->spawn.y * wall_width), wall_width, 0x00FF00);
	draw_rectangle(&cub->mlx_data->img_data, square);
...
static void	draw_player(t_cub *cub, int wall_width)
{
	...
	square = set_square(set_point((cub->player->x - PLAYER_OFFSET) * wall_width,
								  (cub->player->y - PLAYER_OFFSET) * wall_width), wall_width * 2 * PLAYER_OFFSET, 0xFFFFFF);
	draw_rectangle(&cub->mlx_data->img_data, square);
...
static void draw_player_rotation(t_cub *cub, int wall_width)
{
	...
	(void) wall_width;
	p_rot.x = cub->player->x * wall_width + cosf(cub->player->rotation) * wall_width;
	p_rot.y = cub->player->y * wall_width + sinf(cub->player->rotation) * wall_width;
	p_pos.x = cub->player->x * wall_width;
	p_pos.y = cub->player->y * wall_width;
	draw_line(&cub->mlx_data->img_data, p_pos, p_rot, 0xFF);
}
...
static void draw_player_fov(t_cub *cub, int wall_width)
{
	...
	draw_line(&cub->mlx_data->img_data, p_pos, p_fovr, 0xFF00FF);
	draw_line(&cub->mlx_data->img_data, p_pos, p_fovl, 0xFF00FF);
}
