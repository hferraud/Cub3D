...
static void			mouse_rotation(t_cub *cub);
...
static void	player_rotation_update(t_cub *cub)
{
	...
	{
		if (is_key_pressed(KEY_LEFT, cub))
			cub->player_data.player.rotation
				= fvector_rotate(cub->player_data.player.rotation,
					-PLAYER_ROTATION);
		...
		cub->player_data.camera
			= fvector_rotate(cub->player_data.player.rotation, M_PI_2);
	}
	mouse_rotation(cub);
}
...
static void	mouse_rotation(t_cub *cub)
{
	...
	if (!cub->mlx_data->mouse_disable && x != WIN_WIDTH / 2)
	{
		if (!cub->mlx_data->mouse_disable && x != WIN_WIDTH / 2)
		{
			cub->player_data.player.rotation
				= fvector_rotate(cub->player_data.player.rotation,
					PLAYER_ROTATION / 100.0f * (x - (WIN_WIDTH / 2)));
			cub->player_data.camera
				= fvector_rotate(cub->player_data.player.rotation, M_PI_2);
		}
	}
