...
int	mlx_sprite_init(t_cub *cub)
{
	...
	{
		sprite = &cub->mlx_data->sprite[index];
		sprite->img_data.img
			= mlx_xpm_file_to_image(cub->mlx_data->mlx_ptr,
				cub->map.path[index], &sprite->width,
				&sprite->height);
...
static int	error_sprite_init(t_cub *cub)
{
	...
	while (index < NB_TEXTURE)
	{
		free(cub->map.path[index]);
		cub->map.path[index] = NULL;
		if (cub->mlx_data->sprite[index].img_data.img)
			mlx_destroy_image(cub->mlx_data->mlx_ptr,
				cub->mlx_data->sprite[index].img_data.img);
		cub->mlx_data->sprite[index].img_data.img = NULL;
		index++;
	}
